...
class Domain_Invariant_Dataset(Dataset):
    ...
    def read_frame(self, frame):
        ...
        if mod in ['infra1', 'infra2', 'rgb', 'depth']:
            if frame.endswith('.npy'):
                try:
                    ...
                    print(f"[Warning] Failed to read npy image {frame}: {e}, assigning zero image.")
                    data = np.zeros((480, 640, 3), dtype=np.uint8)
            else:
                data = cv2.imread(frame)
                if data is None or data.ndim != 3:
                    print(f"[Warning] Failed to read image {frame}, assigning zero image.")
                    data = np.zeros((480, 640, 3), dtype=np.uint8)
            ...
            try:
                with open(frame, 'rb') as f:
                    raw_data = f.read()
                    data = np.frombuffer(raw_data, dtype=np.float64).reshape(-1, 3)
            except:
                print(f"[Warning] Lidar read failed: {frame}, assigning default")
            ...
            try:
                with open(frame, 'rb') as f:
                    raw_data = f.read()
                    data = np.frombuffer(raw_data, dtype=np.float64).reshape(-1, 5)
            except:
                print(f"[Warning] mmwave read failed: {frame}, assigning default")
                data = np.zeros((1, 5), dtype=np.float64)
    
        elif mod == 'wifi-csi':
            try:
                mat = scio.loadmat(frame)
                data = mat.get('CSIamp', None)
                if data is None or data.ndim != 3:
                    raise ValueError("Invalid CSIamp format")
                ...
                print(f"[Warning] wifi-csi read failed {frame}: {e}, assigning default")
                data = np.zeros((30, 3, 10), dtype=np.float32)
    
        else:
            raise ValueError(f"Found unseen modality: {mod}")
        ...
        for mod in item['modality']:
            ...
            if os.path.isfile(data_path):
                try:
                    ...
                    # 强制校验 shape，防止空数组穿透
                    if isinstance(data_mod, np.ndarray) and data_mod.size == 0:
                        print(f"[Warning] Empty data for {mod}, assigning default.")
                        if mod in ['mmwave']:
                            data_mod = np.zeros((1, 5), dtype=np.float64)
                        elif mod in ['lidar']:
                            data_mod = np.zeros((1, 3), dtype=np.float64)
                        elif mod in ['wifi-csi']:
                            data_mod = np.zeros((30, 3, 10), dtype=np.float32)

                    sample['input_' + mod] = data_mod

                except Exception as e:
                    print(f"[Warning] Exception processing {mod} at {data_path}: {e}")
                    if mod == 'mmwave':
                        sample['input_' + mod] = np.zeros((1, 5), dtype=np.float64)
                    elif mod == 'lidar':
                        sample['input_' + mod] = np.zeros((1, 3), dtype=np.float64)
                    elif mod == 'wifi-csi':
                        sample['input_' + mod] = np.zeros((30, 3, 10), dtype=np.float32)
            else:
